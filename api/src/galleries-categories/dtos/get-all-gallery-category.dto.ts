...
export class GetAllGalleryCategoriesQueryDto extends PageQuery {
    @ApiPropertyOptional({ description: 'Terme de recherche pour filtrer les catégories par nom' })
    search?: string;
}
...
export class GetAllGalleriesCategoriesResponseDto {
    ...
    constructor(galleryCategory: GalleryCategory) {
        ...
        this.childrens = galleryCategory.childrens.map(child => new GetAllGalleriesCategoriesResponseDto(child));
    }
}

export const mapFromGalleryCategoryToGetAllGalleriesCategoriesResponseDto = (galleriesCategories: GalleryCategory[]): GetAllGalleriesCategoriesResponseDto[] => {
    return galleriesCategories.map(galleryCategory => new GetAllGalleriesCategoriesResponseDto(galleryCategory));
}